...
/// the [`Domain::Operation`] item bound pins the domain's own operation family, families generated by
/// `#[derive(Operation)]` require every member payload's [`Operation::Type`] to equal the family's primary type,
/// and the [`InterpretationDriver`] implementations that hand regions to a rule restate `O: Operation<Type = V::Type>`
/// themselves. A mismatched implementation therefore cannot participate in ordinary eager or staged program execution.
/// This does not make such an implementation unreachable. Because this trait and its method are public, downstream code
/// can implement it for a mismatched context and invoke [`interpret`](Self::interpret) directly. Until the strict bound
/// can be restored, direct implementers and callers are responsible for maintaining `Self::Type == C::Type`.
