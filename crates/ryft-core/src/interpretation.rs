...
/// composite eager dispatcher in [`backends::array_programs`](crate::backends::array_programs). The invariant is
/// enforced on every path that can actually reach [`interpret`](Self::interpret), not on this declaration. [`Program`]s
/// and [`ProgramBuilder`](crate::ProgramBuilder)s require `O: Operation<Type = V::Type>`, the [`Domain::Operation`]
/// item bound pins the domain's own operation family, the families generated by `#[derive(Operation)]` require every
/// member payload's [`Operation::Type`] to equal the family's primary type, and the [`InterpretationDriver`]
/// implementations that hand regions to a rule restate `O: Operation<Type = V::Type>` themselves. A mismatched
/// implementation is consequently unreachable dead code rather than an execution hazard, because nothing can construct
/// the [`Program`] or the operation family that would be needed to call it.
