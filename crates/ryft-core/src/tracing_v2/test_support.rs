...
pub(crate) fn assert_reference_scalar_sine_jit_rendering() {
    ...
    let (_, compiled): (f64, Program<ArrayType, f64, ProgramOpRef<f64>, f64, f64>) =
...
pub(crate) fn assert_bilinear_pushforward_rendering() {
    let engine = ArrayScalarEngine::<f64>::new();
    let (_, pushforward): (f64, LinearProgram<ArrayType, f64, (f64, f64), f64, LinearProgramOpRef<f64>>) =
...
pub(crate) fn assert_bilinear_jit_rendering() {
    let engine = ArrayScalarEngine::<f64>::new();
    let (_, compiled): (f64, Program<ArrayType, f64, ProgramOpRef<f64>, (f64, f64), f64>) =
...
pub(crate) fn assert_quadratic_pushforward_rendering() {
    let engine = ArrayScalarEngine::<f64>::new();
    let (_, pushforward): (f64, LinearProgram<ArrayType, f64, f64, f64, LinearProgramOpRef<f64>>) =
