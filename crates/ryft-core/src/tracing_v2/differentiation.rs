...
pub trait DifferentiationContext: Context {
        ...
        LinearOperationOf<Self>: ResidualizedOperation<Self>,
    {
        ...
        Ok(LinearizedProgram::new(output, Pushforward::new(residuals, pushforward_program)))
    }
    ...
    ///
    /// This is the reusable IR-level form of forward-mode differentiation, built on the symbolic core: the program
    /// is linearized *symbolically* once through [`linearize_program`] (via the [`ProgramLinearizableOperation`]
    /// witness), producing a residual-extended primal program and a residualized pushforward program, and the
    /// resulting [`Linearization`] is then evaluated [`at`](Linearization::at) `input_primals` against this context.
    /// The primal side therefore takes on this context's value semantics — an eager context evaluates it concretely
    /// while a staging context (whose values are [`Tracer`]s) splices it into the active trace — and the returned
    /// [`Pushforward`] can be replayed later on arbitrary tangent inputs at the same primal point.
    ///
    /// # Parameters
    ///
    ...
    where
        ...
        O: Clone + ProgramLinearizableOperation<Self>,
