...
/// Projects one composite type slice to the homogeneous member type expected by an inner operation family.
fn project_types<T: Type>(types: &[ArrayProgramType]) -> Result<Vec<T>, TypeError>
where
    for<'t> &'t T: TryFrom<&'t ArrayProgramType, Error = TypeError>,
{
    types.iter().map(|r#type| <&T>::try_from(r#type).cloned()).collect()
}
...
{
    Ok((
        project_types(input_types)?,
        region_interfaces
            .iter()
            .map(|interface| {
                Ok(RegionInterface::new(
                    project_types(interface.input_types())?,
                    project_types(interface.output_types())?,
                    interface.effects(),
                ))
            })
...
/// Lifts homogeneous inferred types back into the array-program type universe.
fn lift_types<T: Into<ArrayProgramType>>(types: Vec<T>) -> Vec<ArrayProgramType> {
    types.into_iter().map(Into::into).collect()
}

/// Lifts optional homogeneous region-input signatures back into the array-program type universe.
fn lift_region_input_types<T: Into<ArrayProgramType>>(
    input_types: Vec<Option<Vec<T>>>,
) -> Vec<Option<Vec<ArrayProgramType>>> {
    input_types.into_iter().map(|types| types.map(lift_types)).collect()
}
...
impl<A: Value<Type = ArrayType>> Operation<ArrayProgramType> for ArrayProgramOperation<A> {
    ...
    fn infer_region_input_types(
        ...
        match self {
            Self::Array(operation) => {
                ...
                Ok(lift_region_input_types(operation.infer_region_input_types(&input_types, &region_interfaces)?))
            }
            Self::Dimension(operation) => {
                let (input_types, region_interfaces) = project_operation_boundary(input_types, region_interfaces)?;
                Ok(lift_region_input_types(operation.infer_region_input_types(&input_types, &region_interfaces)?))
    ...
    fn infer_output_types(
        ...
        match self {
            Self::Array(operation) => {
                let (input_types, region_interfaces) = project_operation_boundary(input_types, region_interfaces)?;
                Ok(lift_types(operation.infer_output_types(&input_types, &region_interfaces)?))
            }
            Self::Dimension(operation) => {
                let (input_types, region_interfaces) = project_operation_boundary(input_types, region_interfaces)?;
                Ok(lift_types(operation.infer_output_types(&input_types, &region_interfaces)?))
