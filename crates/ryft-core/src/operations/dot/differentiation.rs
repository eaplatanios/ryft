...
impl_differentiable_operation! {
        ...
        C::Value: ConvertElementType + Dot + Add,
    {
        |operation, context, _driver, inputs| {
            ...
            let left = &inputs[0];
            let right = &inputs[1];
            let stage_dot = |left: &C::Value, right: &C::Value| {
                match (operation.accumulation_type(), operation.output_sharding()) {
                    (Some(accumulation_type), _) => {
                        left.dot_with_accumulation_type(right, operation.dimensions(), accumulation_type)
                    }
                    (None, Some(output_sharding)) => {
                        left.dot_with_output_sharding(right, operation.dimensions(), output_sharding)
                    }
    ...
    where
        ...
        O: From<ConvertElementTypeOperation<ArrayType>> + From<DotOperation>,
    {
                ...
                (left_is_linear, _) => {
                    ...
                    let contribution = match &outputs[0] {
                        ...
                        MaybeZero::Value(output_cotangent) => {
                            ...
                            let adjoint_output_sharding =
                                inputs[linear_index].r#type().sharding().map(Sharding::cotangent);
                            let adjoint = if left_is_linear {
                                // Known RHS: linear LHS cotangent is `dot(cotangent, rhs; adjoint_right)`.
                                let dimensions =
                                    adjoint_dimensions_for_right_dot(operation.dimensions(), right_rank, left_rank);
...
impl_differentiable_operation! {
        ...
        C::Value: ConvertElementType + RaggedDot + Add,
    {
        |operation, context, _driver, inputs| {
            ...
            let primal = lhs.primal().ragged_dot_general(rhs.primal(), group_sizes, operation.dimensions())?;
            ...
            let apply = |lhs: &C::Value, rhs: &C::Value| {
                lhs.ragged_dot_general(rhs, &group_sizes, operation.dimensions()).map_err(DifferentiationError::from)
            };
    ...
    where
        V: Value<Type = ArrayType>,
        O: From<ConvertElementTypeOperation<ArrayType>>
            ...
            + From<crate::operations::manipulation::TransposeOperation>,
    {
        |operation, context, _driver, inputs, outputs, accumulators| {
            ...
            let mode = operation.dimensions().mode(inputs[0].r#type().rank())?;
