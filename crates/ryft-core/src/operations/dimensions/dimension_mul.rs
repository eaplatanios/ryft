...
define_arithmetic_dimension_capability!(
    ...
    /// [`DimensionMul::dimension_mul`] is the fallible counterpart to [`std::ops::Mul`];
    ...
    /// let result = DimensionValue::constant(3)?.dimension_mul(&DimensionValue::constant(4)?)?;
    ...
    fn test_dimension_mul_operation() {
        ...
        assert_eq!(
            DimensionValue::constant(7)
                .unwrap()
                .dimension_mul(&DimensionValue::constant(3).unwrap())
                .unwrap()
                .extent(),
            21,
        );
