...
fn test_dot_product_attention_interpretation() {
    ...
    let query = Array::from_elements::<f64>(ArrayType::new_static(DataType::F64, [1, 1, 1]), &[1.0e8]).unwrap();
    let key =
        Array::from_elements::<f64>(ArrayType::new_static(DataType::F64, [2, 1, 1]), &[1.0, 1.0 + 1.0e-8]).unwrap();
    let value = Array::from_elements::<f64>(ArrayType::new_static(DataType::F64, [2, 1, 1]), &[0.0, 1.0]).unwrap();
    let bias = Array::from_elements::<f64>(ArrayType::new_static(DataType::F64, [2]), &[-1.0e8, -1.0e8]).unwrap();
    ...
    let query = Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [1, 1, 2, 1]), &[1.0, 1.0]).unwrap();
    let grouped_key =
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [1, 2, 1, 1]), &[1.0, 2.0]).unwrap();
    let grouped_value =
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [1, 2, 1, 1]), &[10.0, 20.0]).unwrap();
    let repeated_key =
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [1, 2, 2, 1]), &[1.0, 1.0, 2.0, 2.0]).unwrap();
    let repeated_value =
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [1, 2, 2, 1]), &[10.0, 10.0, 20.0, 20.0])
            .unwrap();
...
fn test_dot_product_attention_batching() {
    ...
    let key = ArrayBatch::new(
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [2, 2, 1, 1]), &[1.0, 2.0, 1.0, 2.0]).unwrap(),
        BatchAxis::new(0),
    )
    .unwrap();
    let value = ArrayBatch::new(
        Array::from_elements::<f32>(ArrayType::new_static(DataType::F32, [2, 2, 1, 1]), &[3.0, 9.0, 4.0, 10.0])
            .unwrap(),
        BatchAxis::new(0),
    )
    .unwrap();
