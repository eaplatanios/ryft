...
impl<T: Type, V: Clone + Value<T> + Sub<Output = V>> InterpretableOperation<T, V> for SubOperation
where
    Self: Operation<T>,
{
    #[inline]
...
mod tests {
    ...
    fn test_sub() {
        ...
        assert_eq!(
            InterpretableOperation::<DataType, f64>::interpret(&operation, &mut (), &[2.0, 3.5]),
            Ok(vec![-1.5])
        );
        assert_eq!(
