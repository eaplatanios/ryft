...
mod tests {
    ...
    fn test_device_put_visualizes_uneven_1d_partitioning() {
        ...
        assert_eq!(
            array.sharding().visualize(array.shape().as_slice(), &mesh, false),
            Ok(indoc! {"
                ┌─────┬─────┐
                │  0  │  1  │
                └─────┴─────┘
    ...
    fn test_array_put_reshards_fully_addressable_array() {
        ...
        assert_eq!(
            moved_array.sharding().visualize(moved_array.shape().as_slice(), &target_mesh, false),
            Ok(indoc! {"
                ┌─────┬─────┐
                │  0  │  1  │
                └─────┴─────┘
    ...
    fn test_array_put_copies_matching_local_shards_without_full_source_addressability() {
        ...
        assert_eq!(
            copied_array.sharding().visualize(copied_array.shape().as_slice(), &mesh, false),
    ...
    fn test_device_put_broadcasts_root_placement_over_array_tuple() {
        ...
        assert_eq!(
            moved_arrays.0.sharding().visualize(moved_arrays.0.shape().as_slice(), &target_mesh, false),
            Ok(indoc! {"
                ┌─────┬─────┐
                │  0  │  1  │
                └─────┴─────┘
        ...
        assert_eq!(
            moved_arrays.1.sharding().visualize(moved_arrays.1.shape().as_slice(), &target_mesh, false),
            Ok(indoc! {"
                ┌─────┬─────┐
                │  0  │  1  │
                └─────┴─────┘
