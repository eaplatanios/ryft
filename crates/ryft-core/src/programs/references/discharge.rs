...
impl ReferenceDischargeRegionBoundary {
    ...
    ///     [`ReferenceDischargeContext::boundary_allocation`], which validates that each input carries the complete
...
/// derived by the shared [`ReferenceAnalysis`] traversal from generic operation effects and region provenance. Unlike
/// cached structural analysis, it substitutes this caller's allocation bindings before validating reference identities
/// and analyzes each attachment under its own capture scope. Allocations created inside the closure are deliberately
/// absent as they belong to no caller and cross no boundary.
///
...
impl ReferenceDischargeRegionSummary {
    ...
    ///
    /// The shared [`ReferenceAnalysis`] traversal uses [`Operation::effects`], [`Operation::input_region_provenance`],
    /// and [`Operation::output_region_provenance`] to follow allocations and aliases. An output constrained by
    /// [`Operation::reference_output_identity_input`] must agree with every region output it forwards. Allocations
    /// created within the closure retain their identities while nested boundaries are checked, then are omitted from
    /// this caller-facing summary. In particular, allocating a reference in an outer region does not exempt its
    /// accesses in a nested region from [`Operation::allows_reference_access_through_region_input`]. A region may
    ...
    ///
    /// Only [`RegionRole::Computation`](crate::RegionRole::Computation) regions contribute to the summary. Dormant
    /// [`RegionRole::Rule`](crate::RegionRole::Rule) regions are inputs to later transforms. Those transforms bind and
    /// validate their reference inputs separately. [`Operation::region_capture_input_count`] determines whether the
    /// region inherits `captures` or binds a fresh scope from its leading inputs. Reference constants used by an
    /// instruction or region output resolve through that scope. Unused constants contribute no reachability,
    /// matching [`RegionRef::interpret_with`].
    ///
    /// # Parameters
    ///
    ///   - `operation`: [`Operation`] the region is attached to, supplying its capture and access contracts.
    ...
    /// # Errors
    ///
    /// Returns [`ProgramError::InvalidInputCount`] when `inputs` has the wrong length, or
    /// [`ProgramError::MalformedProgram`] for inconsistent input bindings, the attaching operation's
    /// capture prefix, or its access policy. Shared reference validation errors are preserved through
    /// [`ReferenceError::Analysis`](crate::ReferenceError::Analysis), including invalid nested scopes,
    /// output identities, views, escaping allocations, and consumption across region boundaries.
    pub fn new<V: Value, O: Operation<Type = V::Type>, Owner: Operation>(
        ...
        // must share that representative before the analysis checks output identities, not merely in the final map.
        let mut roots = BTreeMap::new();
        let inputs = inputs
            .iter()
            .enumerate()
            .map(|(input_index, allocation)| {
                allocation.map(|allocation| {
                    *roots.entry(allocation).or_insert(ReferenceRoot::RegionInput { region: region.id(), input_index })
                })
            })
            .collect::<Vec<_>>();
        let capture_count = operation.region_capture_input_count(region_index);
        let captures = match capture_count {
            Some(count) => inputs.get(..count).ok_or_else(|| ProgramError::MalformedProgram(format!(
                "reference discharge cannot establish a capture prefix of {} for region `{}`, which declares {} inputs",
                count, region.id(), inputs.len(),
            )))?.to_vec(),
            None => {
        ...
        let summary = ReferenceAnalysis::summarize_boundary(region, &inputs, &captures, capture_count.unwrap_or(0))?;
        let allocations = roots.into_iter().map(|(allocation, root)| (root, allocation)).collect::<BTreeMap<_, _>>();

        // The shared traversal checks nested access policies and local allocation ownership. The attaching operation
        // is outside that traversal, so check its policy here, only for roots supplied by this caller.
        let accessed_allocations = summary
            .accesses
            .into_iter()
            .filter_map(|(root, modes)| allocations.get(&root).map(|allocation| (*allocation, modes)))
            .collect::<BTreeMap<_, _>>();
        if let Some((allocation, mode)) = accessed_allocations
            .iter()
        ...
        Ok(Self {
            reached_allocations: summary
                .reached
                .into_iter()
                .filter_map(|root| allocations.get(&root).copied())
                .collect(),
            ...
            for position in 0..=source_output_count {
                ...
                while groups.peek().is_some_and(|group| group.position() == position) {
                    for output in groups.next().unwrap().sources() {
                        ...
                        output_ids.push(
                            region_context
                                .boundary_value(&ReferenceDischargeValue::Reference(
                                    region_context.allocation_reference(region_allocation)?,
                                ))?
                                .atom_id()?,
                        );
                    }
...
impl<C: Domain, P: ReferenceDischargePolicy<C>> ReferenceDischargeContext<C, P> {
    ...
    /// rule invocation through [`EmptyRegionDriver`]) has no source program location and is always discharged as no
    ...
    pub fn boundary_allocation(
        ...
        let ReferenceDischargeValue::Reference(reference) = value else {
            return Ok(None);
        };
        let allocation = reference.allocation_id();
        let entry = self.allocation_entry(allocation)?;
    ...
    /// Returns the value used at a rewritten region boundary which is either the current immutable state of a
    /// discharged allocation, the destination reference of a preserved allocation, or the supplied non-reference value
    ...
    /// # Errors
    ///
    /// Returns [`ProgramError::MalformedProgram`] when the value's allocation is not live.
    pub fn boundary_value(&self, value: &ReferenceDischargeValue<C, P>) -> Result<C::Value, ProgramError> {
    ...
    /// # Parameters
    ///
    ///   - `summary`: [`ReferenceDischargeRegionSummary`] of the region closures, which states whether
    ///     `allocation` is mutated.
    ///   - `widening`: [`ReferenceDischargeBoundaryWidening`] computed from `summary`, which states whether
    ///     `allocation` is threaded as state.
    ///   - `allocation`: [`ReferenceDischargeAllocationId`] of the caller allocation that the boundary output denotes.
    ///   - `output`: Destination value the rebuilt operation returned at that boundary position.
    ///
    /// # Errors
    ///
    /// Propagates the liveness and type failures of [`set_discharged_state`](Self::set_discharged_state) that is
    /// invoked internally by this function.
    #[inline]
...
>(
    ...
    let forwarded_allocations =
        forwarded.iter().map(|input| context.boundary_allocation(input)).collect::<Result<Vec<_>, _>>()?;
...
mod tests {
    ...
    enum ListOperation {
        ...
        Call,
        ScopedCall {
            /// Leading inputs that establish a new capture scope, or none to inherit the outer scope.
            capture_count: Option<usize>,
            /// Sole permitted access mode, or none to permit all modes.
            allowed: Option<ReferenceAccessMode>,
            /// Whether each reference output must preserve the corresponding input's identity.
            identity: bool,
            /// Whether the attached region is a rule for later transforms rather than an executed computation.
            dormant: bool,
        },
    }
    ...
    impl Operation for ListOperation {
        ...
        fn name(&self) -> &'static str {
            match self {
                ...
                Self::Call => "list.call",
                Self::ScopedCall { .. } => "test.scoped_call",
            }
        }
    ...
    impl<C: Domain<Type = ListIrType, Value = ListIrValue>> InterpretableOperation<C> for ListOperation {
        fn interpret<D: InterpretationDriver<C>>(
            ...
            match self {
                Self::Call | Self::ScopedCall { .. } => driver.interpret_region(context, 0, inputs.to_vec()),
                Self::Add => {
                    check_count!("input", inputs, 2, ProgramError);
    ...
    /// Builds a caller-local allocation passed to one nested region, optionally named there through a capture.
    fn summary_region_with_local_reference(
        operation: ListOperation,
        access: ListOperation,
        captured: bool,
        ...
        let mut nested = ProgramBuilder::<ListCapture, ListOperation>::new();
        let input = nested.add_input(reference_type.clone());
        let reference = if captured { nested.add_constant(ListCapture::new(0, reference_type)) } else { input };
        let output = nested.add_instruction(access, Vec::new(), vec![reference], None).unwrap()[0];
        let nested = nested
        ...
        let output = builder.add_instruction(operation, vec![nested], vec![local], None).unwrap()[0];
        builder.build(vec![output], vec![Placeholder], vec![Placeholder]).unwrap()
    }

    /// Builds a region that passes capture two to a callee which ignores it and forwards a non-reference value.
    fn region_forwarding_an_unread_capture() -> Program<ListIrValue, ListOperation, Vec<ListIrValue>, Vec<ListIrValue>>
    {
    ...
    /// Summarizes a region that writes one caller allocation, reads a preserved one, and reads a third allocation
    /// whose snapshot it returns. Returns the context, allocations in binding order, and summary under `list.call`.
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_validates_input_bindings() {
        let mut builder = ProgramBuilder::<ListIrValue, ListOperation>::new();
        builder.add_input(ListIrType::Reference(ReferenceType::new(ListType { length: 2 })));
        builder.add_input(ListIrType::List(ListType { length: 2 }));
        let program = builder
        ...
        // The boundary must have one entry per input, and only reference inputs carry allocation identities.
        assert_eq!(
            ReferenceDischargeRegionSummary::new(&ListOperation::Call, 0, region, &[], context.captures()),
            Err(ProgramError::InvalidInputCount { expected: 2, actual: 0 }),
        );
        assert_eq!(
            ReferenceDischargeRegionSummary::new(&ListOperation::Call, 0, region, &[None, None], context.captures()),
            Err(ProgramError::MalformedProgram(format!(
                "reference discharge allocation binding for region `{}` input 0 does not match its reference type",
                region.id(),
            ))),
        );
        assert_eq!(
            ...
            Err(ProgramError::MalformedProgram(format!(
                "reference discharge allocation binding for region `{}` input 1 does not match its reference type",
                region.id(),
            ))),
        );
        assert_eq!(
            ReferenceDischargeRegionSummary::new(
                &ListOperation::ScopedCall { capture_count: Some(3), allowed: None, identity: false, dormant: false },
                0,
                region,
                &[Some(allocation), None],
                context.captures(),
            ...
            Err(ProgramError::MalformedProgram(format!(
                "reference discharge cannot establish a capture prefix of 3 for region `{}`, which declares 2 inputs",
                region.id(),
            ))),
        );
    }
    ...
    fn test_reference_discharge_region_summary_new_resolves_input_capture_aliases() {
        ...
        let captured = builder.add_constant(ListCapture::new(0, reference_type));
        let program = builder
            .build::<Vec<ListCapture>, Vec<ListCapture>>(vec![input, captured], vec![Placeholder], vec![Placeholder; 2])
            .unwrap();
        let context = ListDischargeContext::new(ListDestination::new());
        let first = context
            .bind_discharged(ReferenceType::new(ListType { length: 2 }), ListIrValue::List(vec![1, 2]))
        ...
        // A captured alias uses the input's representative; a different captured allocation keeps its own identity.
        let summary = ReferenceDischargeRegionSummary::new(
            &ListOperation::Call,
            0,
            program.entry_region_ref(),
            &[Some(first)],
            &ReferenceDischargeCaptureScope::new(vec![Some(first)]),
        )
        .unwrap();
        assert_eq!(summary.reached_allocations().collect::<Vec<_>>(), vec![first]);
        assert_eq!(summary.output_allocations(), &[Some(first), Some(first)]);
        let summary = ReferenceDischargeRegionSummary::new(
            &ListOperation::Call,
            0,
            program.entry_region_ref(),
            &[Some(first)],
            &ReferenceDischargeCaptureScope::new(vec![Some(second)]),
        )
        .unwrap();
        assert_eq!(summary.reached_allocations().collect::<Vec<_>>(), vec![first, second]);
        assert_eq!(summary.output_allocations(), &[Some(first), Some(second)]);
    }

    #[test]
    fn test_reference_discharge_region_summary_new_ignores_dead_reference_constants() {
        // Replay never materializes the unused reference, so it needs neither a capture binding nor a state input.
        let mut builder = ProgramBuilder::<ListIrValue, ListOperation>::new();
        builder.add_constant(ListIrValue::Reference(ReferenceType::new(ListType { length: 2 })));
        let value = builder.add_constant(ListIrValue::List(vec![1, 2]));
        let program = builder
            .build::<Vec<ListIrValue>, Vec<ListIrValue>>(vec![value], Vec::new(), vec![Placeholder])
            .unwrap();
        assert_eq!(
            ReferenceDischargeRegionSummary::new(
                &ListOperation::Call,
                ...
                program.entry_region_ref(),
                &[],
                &ReferenceDischargeCaptureScope::default(),
            ),
            Ok(ReferenceDischargeRegionSummary { output_allocations: vec![None], ..Default::default() }),
        );
    }

    #[test]
    fn test_reference_discharge_region_summary_new_allows_local_consumption() {
        // Region-input restrictions apply to entering references, not to an allocation created and consumed locally.
        let mut builder = ProgramBuilder::<ListIrValue, ListOperation>::new();
        let value = builder.add_input(ListIrType::List(ListType { length: 2 }));
        let local = builder.add_instruction(ListOperation::ReferenceNew, Vec::new(), vec![value], None).unwrap()[0];
        let frozen = builder.add_instruction(ListOperation::Freeze, Vec::new(), vec![local], None).unwrap()[0];
        let program = builder
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_resolves_local_captures() {
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_validates_nested_local_accesses() {
        // Local to the outer closure does not mean local to its callee: the callee's input policy still applies.
        let region = summary_region_with_local_reference(
            ListOperation::ScopedCall {
                capture_count: None,
                allowed: Some(ReferenceAccessMode::Write),
                identity: false,
                dormant: false,
            },
            ListOperation::Read,
        ...
        let local = ReferenceRoot::Allocation { instruction: InstructionId::new(entry.id(), 0), output_index: 0 };
        assert_eq!(
            ReferenceDischargeRegionSummary::new(
                &ListOperation::Call,
                0,
            ...
            Err(ReferenceAnalysisError::DisallowedRegionAccess {
                operation: "test.scoped_call",
                instruction: InstructionId::new(entry.id(), 1),
                region_index: 0,
                root: local,
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_rejects_nested_local_consumption() {
        // Only the allocation's creating region may consume it, even though the whole outer closure owns it.
        let region = summary_region_with_local_reference(ListOperation::Call, ListOperation::Freeze, false);
        let entry = region.entry_region_ref();
        let local = ReferenceRoot::Allocation { instruction: InstructionId::new(entry.id(), 0), output_index: 0 };
        assert_eq!(
            ReferenceDischargeRegionSummary::new(
            ...
            Err(ReferenceAnalysisError::ConsumptionOutsideCreationScope {
                operation: "list.freeze",
                instruction: InstructionId::new(entry.instructions()[1].regions()[0], 0),
                region: entry.instructions()[1].regions()[0],
                root: local,
            }
            .into()),
        );
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_validates_output_identity() {
        // The callee swaps two references, contradicting the attaching operation's positional identity contract.
        let reference_type = ListIrType::Reference(ReferenceType::new(ListType { length: 2 }));
        let mut nested = ProgramBuilder::<ListIrValue, ListOperation>::new();
        let first = nested.add_input(reference_type.clone());
        let second = nested.add_input(reference_type.clone());
        let nested = nested
            .build::<Vec<ListIrValue>, Vec<ListIrValue>>(
                vec![second, first],
                vec![Placeholder; 2],
        ...
        let outputs = builder
            .add_instruction(
                ListOperation::ScopedCall { capture_count: None, allowed: None, identity: true, dormant: false },
                vec![nested],
                vec![first, second],
                None,
            )
            ...
            .to_vec();
        let region = builder
            .build::<Vec<ListIrValue>, Vec<ListIrValue>>(outputs, vec![Placeholder; 2], vec![Placeholder; 2])
            .unwrap();
        let context = ListDischargeContext::new(ListDestination::new());
        let first = context
            .bind_discharged(ReferenceType::new(ListType { length: 2 }), ListIrValue::List(vec![1, 2]))
        ...
        assert_eq!(
            ReferenceDischargeRegionSummary::new(
                ...
                0,
                region.entry_region_ref(),
                &[Some(first.allocation_id()), Some(second.allocation_id())],
                context.captures(),
            ),
            Err(ReferenceAnalysisError::ReferenceRootMismatch {
                operation: "test.scoped_call",
                instruction: InstructionId::new(region.entry_region_ref().id(), 0),
                output_index: 0,
                region_index: 0,
                region_output_index: 0,
                expected: ReferenceRoot::RegionInput { region: region.entry_region_ref().id(), input_index: 0 },
                actual: ReferenceRoot::RegionInput { region: region.entry_region_ref().id(), input_index: 1 },
            }
            .into()),
        );
        // The same output declarations are consistent when both formal inputs name the same caller allocation.
        let summary = ReferenceDischargeRegionSummary::new(
            &ListOperation::Call,
            0,
            region.entry_region_ref(),
            &[Some(first.allocation_id()), Some(first.allocation_id())],
            context.captures(),
        )
        .unwrap();
        assert_eq!(summary.output_allocations(), &[Some(first.allocation_id()), Some(first.allocation_id())]);
    }
    ...
    fn test_reference_discharge_region_summary_new_rejects_a_consuming_closure() {
        ...
        assert_eq!(
            ...
            Err(ReferenceAnalysisError::ExternalReferenceConsumption {
                operation: "list.freeze",
                instruction: InstructionId::new(program.entry_region_ref().id(), 0),
                root: ReferenceRoot::RegionInput { region: program.entry_region_ref().id(), input_index: 0 },
    ...
    fn test_reference_discharge_region_summary_new_rejects_unbound_reference_constants() {
        ...
        assert_eq!(
            ...
            Err(ReferenceAnalysisError::InvalidReferenceCapture {
                region: reads.entry_region_ref().id(),
                atom: captured,
                capture_index: 2,
                capture_count: 0,
            }
        ...
        assert_eq!(
            ...
            Err(ReferenceAnalysisError::InvalidReferenceCapture {
                region: returns.entry_region_ref().id(),
                atom: captured,
                capture_index: 2,
                capture_count: 0,
            }
    ...
    #[test]
    fn test_reference_discharge_region_summary_new_reaches_unread_captures() {
        ...
        let program = region_forwarding_an_unread_capture();
        let context = ListDischargeContext::new(ListDestination::new());
        let allocated = ReferenceDischargeValue::from(
            context
                .bind_discharged(ReferenceType::new(ListType { length: 2 }), ListIrValue::List(vec![1, 2]))
        ...
        assert!(!summary.is_mutated(allocation));
        assert_eq!(summary.reached_allocations().collect::<Vec<_>>(), vec![allocation]);
    }
    ...
    #[test]
    fn test_reference_discharge_region_summary_reached_allocations() {
        let (_, [written, observed, forwarded], summary) = summarized_region_with_three_allocations();
        assert_eq!(summary.reached_allocations().collect::<Vec<_>>(), vec![written, observed, forwarded]);
        assert_eq!(ReferenceDischargeRegionSummary::default().reached_allocations().collect::<Vec<_>>(), vec![]);
    }

    #[test]
    fn test_reference_discharge_region_summary_accessed_allocations() {
        let (_, [written, observed, forwarded], summary) = summarized_region_with_three_allocations();
        assert_eq!(summary.accessed_allocations().collect::<Vec<_>>(), vec![written, observed, forwarded]);
        assert_eq!(ReferenceDischargeRegionSummary::default().accessed_allocations().collect::<Vec<_>>(), vec![]);
    }

    #[test]
    fn test_reference_discharge_region_summary_access_modes() {
        let (_, [written, observed, _], summary) = summarized_region_with_three_allocations();
        ...
        let absent = ReferenceDischargeAllocationId { index: usize::MAX, ..written };
        assert_eq!(summary.access_modes(absent).collect::<Vec<_>>(), vec![]);
    }
    ...
    fn test_reference_discharge_region_summary_output_allocations() {
        ...
        assert_eq!(ReferenceDischargeRegionSummary::default().output_allocations(), &[]);
    }

    #[test]
    fn test_reference_discharge_region_summary_is_mutated() {
        let (_, [written, observed, forwarded], summary) = summarized_region_with_three_allocations();
        assert!(summary.is_mutated(written));
        assert!(!summary.is_mutated(observed));
        assert!(!summary.is_mutated(forwarded));
        assert!(!ReferenceDischargeRegionSummary::default().is_mutated(written));
    }

    #[test]
    fn test_reference_discharge_region_summary_merge() {
        let (_, [first, second, third], _) = summarized_region_with_three_allocations();
        let mut left = ReferenceDischargeRegionSummary {
            reached_allocations: BTreeSet::from([first, second]),
            accessed_allocations: BTreeMap::from([(first, BTreeSet::from([ReferenceAccessMode::Read]))]),
            output_allocations: vec![Some(first), None],
        };
        let right = ReferenceDischargeRegionSummary {
            reached_allocations: BTreeSet::from([first, third]),
            accessed_allocations: BTreeMap::from([
                (first, BTreeSet::from([ReferenceAccessMode::Read, ReferenceAccessMode::Accumulate])),
                (third, BTreeSet::from([ReferenceAccessMode::Write])),
            ]),
            output_allocations: vec![Some(third)],
        };
        let original_right = right.clone();

        // Union includes references reached without an access and combines modes without duplicates.
        left.merge(&right);
        assert_eq!(left.reached_allocations().collect::<Vec<_>>(), vec![first, second, third]);
        assert_eq!(left.accessed_allocations().collect::<Vec<_>>(), vec![first, third]);
        assert_eq!(
            left.access_modes(first).collect::<Vec<_>>(),
            vec![ReferenceAccessMode::Read, ReferenceAccessMode::Accumulate]
        );
        assert_eq!(left.access_modes(third).collect::<Vec<_>>(), vec![ReferenceAccessMode::Write]);
        assert_eq!(left.output_allocations(), &[Some(first), None]);
        assert_eq!(right, original_right);

        // Repeated merges and an empty summary do not change the result or replace its output list.
        let merged = left.clone();
        left.merge(&right);
        left.merge(&ReferenceDischargeRegionSummary::default());
    ...
    #[test]
    fn test_recursive_reference_discharge_driver_rebuild_region_threads_unread_captures() {
        let program = region_forwarding_an_unread_capture();
        let context = ListDischargeContext::new(ListDestination::new());
        let allocated = ReferenceDischargeValue::from(
            context
    ...
    fn test_reference_discharge_context_bind_preserved() {
        ...
        assert_eq!(
            context.boundary_value(&ReferenceDischargeValue::Reference(reference.clone())),
    ...
    #[test]
    fn test_reference_discharge_context_boundary_widening_preserves_unread_captures() {
        let program = region_forwarding_an_unread_capture();
    ...
    #[test]
    fn test_reference_discharge_context_boundary_allocation() {
        let context = ListDischargeContext::new(ListDestination::new());
        let reference_type = ReferenceType::new(ListType { length: 2 });
        ...
        assert_eq!(context.boundary_allocation(&value), Ok(None));
        ...
        assert_eq!(context.boundary_allocation(&discharged), Ok(Some(discharged_allocation)));
        let preserved = ReferenceDischargeValue::from(
            context
                .bind_preserved(reference_type.clone(), ListIrValue::Reference(reference_type.clone()))
                .unwrap(),
        ...
        assert_eq!(
            context.boundary_allocation(&discharged),
            Err(ProgramError::MalformedProgram(format!(
    ...
    #[test]
    fn test_reference_discharge_context_boundary_value() {
        let context = ListDischargeContext::new(ListDestination::new());
        let reference_type = ReferenceType::new(ListType { length: 2 });

        // A value input contributes itself.
        let value = ReferenceDischargeValue::Value(ListIrValue::List(vec![7, 8]));
        assert_eq!(context.boundary_value(&value), Ok(ListIrValue::List(vec![7, 8])));
        ...
        let discharged_reference = discharged.try_as_reference("the discharged allocation").unwrap();
        assert_eq!(context.boundary_value(&discharged), Ok(ListIrValue::List(vec![1, 2])));
        context.write(discharged_reference, ListIrValue::List(vec![3, 4])).unwrap();
        assert_eq!(context.boundary_value(&discharged), Ok(ListIrValue::List(vec![3, 4])));
        ...
        assert_eq!(context.boundary_value(&preserved), Ok(destination_reference));

        // A consumed allocation is rejected.
        let discharged_allocation = discharged_reference.allocation_id();
        context.consume(discharged_reference).unwrap();
        assert_eq!(
            context.boundary_value(&discharged),
            Err(ProgramError::MalformedProgram(format!(
                "reference discharge accessed consumed {discharged_allocation}"
            ))),
        );
    ...
    #[test]
    fn test_reference_discharge_context_boundary_value_from_allocation() {
        let context = ListDischargeContext::new(ListDestination::new());
        let reference_type = ReferenceType::new(ListType { length: 2 });
        ...
        assert_eq!(
            context
                .allocation_reference(discharged_allocation)
                .and_then(|reference| context.boundary_value(&ReferenceDischargeValue::Reference(reference))),
        ...
        assert_eq!(
            context
                .allocation_reference(preserved_allocation)
                .and_then(|reference| context.boundary_value(&ReferenceDischargeValue::Reference(reference))),
            Ok(destination_reference),
        );
    ...
    fn test_program_discharge_references_omits_a_dead_constant() {
        ...
        builder
            .add_instruction(ReferenceAddUpdateOperation::new(), Vec::new(), vec![allocation, update], None)
            .unwrap();
        let frozen = builder
            .add_instruction(ReferenceFreezeOperation::new(), Vec::new(), vec![allocation], None)
            .unwrap()[0];
        let source = builder
            .build::<Vec<DischargeValue>, Vec<DischargeValue>>(vec![frozen], vec![Placeholder], vec![Placeholder])
            .unwrap();
    ...
    #[test]
    fn test_program_discharge_references_stages_immutable_array_state() {
        let mut builder = ProgramBuilder::<DischargeValue, DischargeOperation>::new();
        let initial = builder.add_input(ArrayType::scalar(DataType::F32).into());
    ...
    #[test]
    fn test_program_discharge_references_generated_short_state_programs_match_eager_and_immutable_oracles() {
        ...
        // Every bounded read/write/swap/accumulate sequence must agree with both an independent discharge_scalar oracle
        // and the eager reference interpreter, which pins the state-threading rewrite over all short primitive
        // orderings.
        for length in 0usize..=3 {
            for code in 0..4usize.pow(length as u32) {
                ...
                // Every generated program is discharged and then checked against the eager and hand-written immutable
                // oracles.
                let discharged = source.discharge_references(0).unwrap();
    ...
    #[test]
    fn test_program_discharge_references_orders_external_state_by_input() {
        let mut builder = ProgramBuilder::<DischargeValue, DischargeOperation>::new();
        ...
        // Access the second allocation first so metadata order cannot accidentally follow access order.
        let second_snapshot =
        ...
        // the complete external-reference binding metadata, including its serialized form.
        let repeated = source.clone().discharge_references(0).unwrap();
        let discharged = source.discharge_references(0).unwrap();
        assert_eq!(discharged.program().to_string(), repeated.program().to_string());
        assert_eq!(discharged.external_reference_bindings(), repeated.external_reference_bindings());
        assert_eq!(
            serde_json::to_string(discharged.external_reference_bindings()).unwrap(),
            serde_json::to_string(repeated.external_reference_bindings()).unwrap(),
        );
        ...
        // receives a hidden final-state output after the public prefix.
        assert_eq!(
            discharged.external_reference_bindings(),
            &[
        ...
        assert_eq!(
            format!("{:?}", discharged.external_reference_bindings()),
            concat!(
    ...
    fn test_program_discharge_references_widens_positional_callee() {
        ...
        let mut callee_builder = ProgramBuilder::<DischargeValue, CallingOperation>::new();
        let reference = callee_builder.add_input(ReferenceType::new(ArrayType::scalar(DataType::F32)).into());
        let replacement = callee_builder.add_input(ArrayType::scalar(DataType::F32).into());
        let old = callee_builder
            ...
            .unwrap()[0];
        let callee = callee_builder
            .build::<Vec<DischargeValue>, Vec<DischargeValue>>(vec![old], vec![Placeholder; 2], vec![Placeholder])
            .unwrap();

        let mut builder = ProgramBuilder::<DischargeValue, CallingOperation>::new();
        ...
        let allocation = builder
            ...
            .unwrap()[0];
        let old = builder
            .add_instruction(CallingOperation::Call, vec![callee], vec![allocation, replacement], None)
            .unwrap()[0];
    ...
    #[test]
    fn test_program_partially_discharge_references_keeps_kernel_references_beside_discharged_pipeline_state() {
        ...
        let step = builder.add_input(ArrayType::scalar(DataType::F32).into());
    ...
    #[test]
    fn test_program_partially_discharge_references_widens_nothing_for_a_preserved_allocation_a_region_writes() {
        ...
        // write replayed into the rebuilt branch as the operation the source performed. The condition therefore keeps
        // its source boundary exactly.
        let reference_type = ReferenceType::new(ArrayType::scalar(DataType::F32));
    ...
    #[test]
    fn test_closed_program_discharge_references_resolves_transitively_nested_reference_captures() {
        let reference_type = ReferenceType::new(ArrayType::scalar(DataType::F32));
        let predicate_type = ArrayType::scalar(DataType::Boolean);
        let mut leaf_builder = ProgramBuilder::<DischargeCapture, DischargeCaptureOperation>::new();
        let reference = leaf_builder.add_constant(DischargeCapture::new(0, reference_type.into()));
        ...
        let mut middle_builder = ProgramBuilder::<DischargeCapture, DischargeCaptureOperation>::new();
    ...
    fn test_discharge_positional_region_operation_recovers_a_returned_preserved_capture_scoped_allocation() {
        ...
        assert_eq!(context.boundary_value(&results[0]), Ok(destination_reference));
    }
