...
pub use regions::{
    ...
    ReferenceRegionSummary, ReferenceStateWidening, discharge_positional_region_operation,
};
...
use crate::programs::references::discharge::regions::{
    nested_capture_scope, summarize_region_closure, validate_region_accesses,
};
...
/// environment's [`ReferenceRootHandle`]s. In particular, a nested region's formal reference input is
/// invocation-parameterized — the region may be invoked from several call sites — so it names no single caller-owned
/// reference and is deliberately not selectable. Sites resolve internally to roots once discharge starts.
///
...
impl<C: Domain, P: ReferenceDischargePolicy<C>> ReferenceDischargeContext<C, P> {
    ...
    ///
    /// Returns [`ProgramError::MalformedProgram`] when the root is not live or was preserved rather than discharged.
    pub(super) fn is_mutated(&self, root: ReferenceRootHandle) -> Result<bool, ProgramError> {
        self.with_root_entry(root, |entry| match &entry.state {
...
fn replay_preserved_access<C, P>(
...
mod tests {
    ...
    use super::regions::lift_constant;
    use super::*;
