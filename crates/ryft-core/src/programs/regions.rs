...
#[derive(Clone, Debug)]
struct RegionWithMetadata<V: Typed + Parameter, O> {
...
impl<V: Value, O: Operation<Type = V::Type>> RegionWithMetadata<V, O> {
    ...
    #[inline]
    fn new(region: Region<V, O>, sealed_regions: &[Self]) -> Result<Self, ProgramError> {
    ...
    #[inline]
    fn new_preserving_transform_cache(region: Region<V, O>, sealed_regions: &[Self]) -> Result<Self, ProgramError> {
...
pub trait BindingRegionDriver<V: Value, O: Operation<Type = V::Type>>: RegionDriver<V, O> + Sized {
    ...
    /// [`Some`] instantiates their nominal identities from the supplied input types. Importing is _structural_. It
    /// renames stored type metadata without re-inferring instruction outputs or replaying bodies. The staging context
    /// specializes bodies when input bounds or geometry become more precise, and reconciles signatures that depend
    /// on other attached regions before recording the operation.
    fn import_into(
        self,
...
impl<V: Value, O: Operation<Type = V::Type>> BindingRegionDriver<V, O> for ReplayRegionDriver<'_, V, O> {
    fn import_into(
        ...
        // Use it as the ordinary cache key when no renamed source exists. Otherwise, use the type-identity renaming
        // already applied to the cached source arena, for example `m -> m_replayed` from an earlier instruction. The
        // local identity value also gives the borrowed fallback key a lifetime covering every root import.
        ...
        self.roots
            ...
            .map(|(root, input_types)| {
                // This view already includes the enclosing replay type-identity renaming. Formal input instantiation
                ...
                // contain replayed identities. Root, input signature, and replay type-identity renaming all belong
                // in the cache key as equal scalar inputs alone cannot distinguish bodies referring to different
                // dimensions.
                ...
                let imported = if input_renaming.is_identity() {
                    ...
                    // type-identity renaming, including entries created by the direct-import path above.
                    builder.import_region_with_remapping(region, destination.region_mapping(replay_renaming))
                } else {
...
impl<V: Value, O: Operation<Type = V::Type>> Default for RegionReplayMappings<V, O> {
    #[inline]
    fn default() -> Self {
        Self { destinations: RefCell::new(Vec::new()), renamed_sources: RefCell::new(Vec::new()) }
    }
}
...
struct DestinationRegionMapping<V: Value, O: Operation<Type = V::Type>> {
    ...
    region_mappings: Vec<(TypeIdentityRenaming<<V::Type as Type>::Identity>, HashMap<RegionId, RegionId>)>,
    ...
    /// their descendants.
    instantiated_region_mappings: Vec<InstantiatedRegionMapping<V::Type>>,
}
...
struct InstantiatedRegionMapping<T: Type> {
...
mod tests {
    ...
    fn test_replay_region_driver_instantiation_cache_preserves_live_type_identities() {
        ...
        let input = source_builder.add_input(array_type(formal.clone()));
        ...
        let replayed = DimensionVariable::new("replayed", bounds);
        renaming.insert(formal, replayed.clone()).unwrap();
        let renamed_destination = Rc::new(RefCell::new(ProgramBuilder::new()));
        let driver = ReplayRegionDriver::new(source.entry_region_ref(), &roots, &mappings)
            .unwrap()
            .with_type_identity_renaming(&renaming)
            .unwrap();
        assert_eq!(driver.region(0).unwrap().input_types(), vec![array_type(replayed.clone())]);
        let imported = driver.import_into(&renamed_destination, &[None]).unwrap()[0];
        assert_eq!(
            renamed_destination.borrow().region_ref(imported).unwrap().input_types(),
            vec![array_type(replayed)]
        );
    }
    ...
    fn test_replay_region_driver_import_preserves_shared_descendants() {
        ...
        // A replay type-identity renaming must preserve the same arena sharing even when the identities being renamed
        // belong to an earlier instruction and do not occur in these particular nested regions.
        let mut renaming = TypeIdentityRenaming::new();
        let bounds = DimensionBounds::non_negative(Some(8)).unwrap();
        renaming
            .insert(DimensionVariable::new("source", bounds), DimensionVariable::new("replayed", bounds))
            .unwrap();
        let renamed_destination = Rc::new(RefCell::new(ProgramBuilder::new()));
        ...
        let repeated = ReplayRegionDriver::new(program.entry_region_ref(), &roots, &mappings)
            .unwrap()
            .with_type_identity_renaming(&renaming)
            .unwrap();
        ...
        // Equal roots and formal input types do not justify reuse under a different replay type-identity renaming.
