...
impl<V: Typed, O> Region<V, O> {
        ...
        O: Operation<V::Type>,
    {
        ...
        // A definition-position occurrence on a constant establishes one immutable SSA value just like a
        // definition-position instruction result. Process all such definitions before constant references so the
        // atom-table order does not impose an artificial dominance relationship among constants.
...
mod tests {
    ...
    fn test_structural_identity_closure_classifies_forwarded_and_fresh_definitions() {
        ...
        // A constant value is also an SSA definition and may therefore establish its own fresh identity.
