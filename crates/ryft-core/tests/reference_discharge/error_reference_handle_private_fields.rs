...
// accessors are not part of its surface, so the root identity a handle carries stays checked rather than borrowable.
//
...
use ryft_core::{Domain, ReferenceDischargePolicy, ReferenceDischargeReference, ReferenceRootHandle};
