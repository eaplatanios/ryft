...
use crate::{Attribute, Context, Error, FloatType, Location, TryFromWithContext, TypeId, mlir_subtype_trait_impls};
...
impl<'c, 't> FloatAttributeRef<'c, 't> {
    ...
    pub fn type_id() -> Result<TypeId<'static>, Error> {
        unsafe { TypeId::from_c_api(mlirFloatAttrGetTypeID()) }
    }
...
impl<'c, 't> TryFromWithContext<'c, 't, f16> for FloatAttributeRef<'c, 't> {
    fn try_from_with_context(value: f16, context: &'c Context<'t>) -> Result<Self, Error> {
        Ok(context.float_attribute(context.float32_type(), value.to_f64()))
    }
}

impl<'c, 't> TryFromWithContext<'c, 't, bf16> for FloatAttributeRef<'c, 't> {
    fn try_from_with_context(value: bf16, context: &'c Context<'t>) -> Result<Self, Error> {
        Ok(context.float_attribute(context.bfloat16_type(), value.to_f64()))
    }
}

impl<'c, 't> TryFromWithContext<'c, 't, f32> for FloatAttributeRef<'c, 't> {
    fn try_from_with_context(value: f32, context: &'c Context<'t>) -> Result<Self, Error> {
        Ok(context.float_attribute(context.float32_type(), value as f64))
    }
}

impl<'c, 't> TryFromWithContext<'c, 't, f64> for FloatAttributeRef<'c, 't> {
    fn try_from_with_context(value: f64, context: &'c Context<'t>) -> Result<Self, Error> {
        Ok(context.float_attribute(context.float64_type(), value))
    }
}
...
impl<'t> Context<'t> {
    ...
    pub fn float_attribute<'c, T: FloatType<'c, 't>>(&'c self, r#type: T, value: f64) -> FloatAttributeRef<'c, 't> {
        ...
        let handle = unsafe { mlirFloatAttrDoubleGet(*self.handle.borrow(), r#type.to_c_api(), value) };
        FloatAttributeRef { handle, context: self }
    }
    ...
    pub fn checked_float_attribute<'c, T: FloatType<'c, 't>, L: Location<'c, 't>>(
        ...
        value: f64,
        location: L,
    ) -> Result<FloatAttributeRef<'c, 't>, Error> {
        // While this operation can mutate the context (in that it might add an entry to its corresponding
        // uniquing table), we use an immutable borrow here as a mutable borrow would make using this
        // function quite inconvenient/annoying in practice. This should have no negative consequences in
        // terms of safety since MLIR contexts are not thread-safe and in a single-threaded context there
...
mod tests {
    ...
    fn test_float_attribute_type_id() {
        ...
        let float_attribute_id = FloatAttributeRef::type_id().unwrap();
        let float_attribute_1: FloatAttributeRef<'_, '_> = 1.0.try_into_with_context(&context).unwrap();
        let float_attribute_2: FloatAttributeRef<'_, '_> = f16::PI.try_into_with_context(&context).unwrap();
        assert_eq!(float_attribute_1.type_id().unwrap(), float_attribute_2.type_id().unwrap());
        assert_eq!(float_attribute_id, float_attribute_1.type_id().unwrap());
    }
    ...
    fn test_float_attribute() {
        ...
        let attribute: FloatAttributeRef<'_, '_> = std::f64::consts::PI.try_into_with_context(&context).unwrap();
        assert_eq!(&context, attribute.context());
        assert!((attribute.value() - std::f64::consts::PI).abs() < 1e-6);

        // Test with `float64` type.
        let attribute: FloatAttributeRef<'_, '_> = bf16::E.try_into_with_context(&context).unwrap();
        assert_eq!(&context, attribute.context());
    ...
    fn test_float_attribute_display_and_debug() {
        ...
        let attribute: FloatAttributeRef<'_, '_> = 42.0f32.try_into_with_context(&context).unwrap();
