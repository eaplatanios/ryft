...
impl<'c, 't> Float8ExmyTypeRef<'c, 't> {
    ...
    pub fn underlying_type(&self) -> TypeRef<'c, 't> {
        unsafe {
            TypeRef::from_c_api(mlirTpuFloat8EXMYTypeGetUnderlyingType(self.handle), self.context)
                .expect("invalid Mosaic TPU float8 EXMY underlying type")
        }
    }
}
...
impl<'t> Context<'t> {
    ...
    pub fn mosaic_tpu_float8_exmy_type<'c, T: Type<'c, 't>>(&'c self, underlying_type: T) -> Float8ExmyTypeRef<'c, 't> {
...
mod tests {
    ...
    fn test_float8_exmy_type() {
        ...
        let r#type = context.mosaic_tpu_float8_exmy_type(underlying_type);
        assert_eq!(&context, r#type.context());
        assert_eq!(r#type.dialect().namespace().unwrap(), "tpu");
        assert_eq!(r#type.underlying_type(), underlying_type);
    }

    #[test]
    fn test_float8_exmy_type_equality() {
        let context = Context::new();
        let r#type_1 = context.mosaic_tpu_float8_exmy_type(context.float8e4m3fn_type());
        let r#type_2 = context.mosaic_tpu_float8_exmy_type(context.float8e4m3fn_type());
        assert_eq!(r#type_1, r#type_2);

        let r#type_2 = context.mosaic_tpu_float8_exmy_type(context.float8e5m2_type());
    ...
    fn test_float8_exmy_type_display_and_debug() {
        let context = Context::new();
        let r#type = context.mosaic_tpu_float8_exmy_type(context.float8e4m3fn_type());
    ...
    fn test_semaphore_types() {
        let context = Context::new();
        let semaphore_type = context.mosaic_tpu_semaphore_type();
        assert_eq!(&context, semaphore_type.context());
        assert_eq!(semaphore_type.dialect().namespace().unwrap(), "tpu");

        let dma_semaphore_type = context.mosaic_tpu_dma_semaphore_type();
        assert_eq!(&context, dma_semaphore_type.context());
        assert_eq!(dma_semaphore_type.dialect().namespace().unwrap(), "tpu");
    ...
    fn test_semaphore_type_display_and_debug() {
        let context = Context::new();
        test_type_display_and_debug(context.mosaic_tpu_semaphore_type(), "!tpu.semaphore");
        test_type_display_and_debug(context.mosaic_tpu_dma_semaphore_type(), "!tpu.dma_semaphore");
    }
    ...
    fn test_semaphore_type_parsing() {
        let context = Context::new();
        context.load_dialect(DialectHandle::mosaic_tpu());
        assert_eq!(context.parse_type("!tpu.semaphore").unwrap(), context.mosaic_tpu_semaphore_type());
        assert_eq!(context.parse_type("!tpu.dma_semaphore").unwrap(), context.mosaic_tpu_dma_semaphore_type());
    }

    #[test]
    fn test_semaphore_type_casting() {
        let context = Context::new();
        test_type_casting(context.mosaic_tpu_semaphore_type());
        test_type_casting(context.mosaic_tpu_dma_semaphore_type());
    }
}
