...
impl DialectHandle<'_, '_> {
    ...
    pub fn mosaic_tpu() -> Self {
        unsafe { Self::from_c_api(mlirGetDialectHandle__tpu__()).unwrap() }
    }
}
...
mod tests {
    ...
    fn test_mosaic_tpu_dialect() {
        let handle = DialectHandle::mosaic_tpu();
        ...
        assert_eq!(dialect_1.unwrap().namespace().unwrap(), "tpu");

        // Check that comparison works.
        let dialect_2 = context.load_dialect(DialectHandle::mosaic_tpu());
