use crate::{
    DetachedOp, DetachedRegion, DialectHandle, Error, Location, Operation, OperationBuilder, RegionRef, Value,
...
pub trait CaseOperation<'o, 'c: 'o, 't: 'c>: Operation<'o, 'c, 't> {
    ...
    fn branches(&self) -> impl Iterator<Item = Result<RegionRef<'o, 'c, 't>, Error>> {
        self.regions()
    }
}
...
pub fn case<'v, 'c: 'v, 't: 'c, V: Value<'v, 'c, 't>, L: Location<'c, 't>>(
    ...
    OperationBuilder::new("stablehlo.case", location)
        ...
        .and_then(|operation| unsafe {
            operation.cast().ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::case`"))
        })
}
...
pub trait IfOperation<'o, 'c: 'o, 't: 'c>: Operation<'o, 'c, 't> {
    ...
    fn true_branch(&self) -> Result<RegionRef<'o, 'c, 't>, Error> {
        self.region(0)
    }

    /// Returns the `false` branch of this [`IfOperation`].
    fn false_branch(&self) -> Result<RegionRef<'o, 'c, 't>, Error> {
        self.region(1)
    }
}
...
pub fn r#if<'v, 'c: 'v, 't: 'c, V: Value<'v, 'c, 't>, L: Location<'c, 't>>(
    ...
    OperationBuilder::new("stablehlo.if", location)
        ...
        .build()
        .and_then(|operation| unsafe {
            operation.cast().ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::if`"))
        })
}
...
pub fn r#while<'v, 'c: 'v, 't: 'c, V: Value<'v, 'c, 't>, L: Location<'c, 't>>(
    ...
    OperationBuilder::new("stablehlo.while", location)
        ...
        .build()
        .and_then(|operation| unsafe {
            operation.cast().ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::while`"))
        })
}
...
>(
    ...
    OperationBuilder::new("stablehlo.select", location)
        ...
        .build()
        .and_then(|operation| unsafe {
            operation.cast().ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::select`"))
        })
}
        ...
        module
            ...
            .append_operation({
                ...
                let value_0_op = branch_0_block
                    ...
                    .unwrap();
                branch_0_block
                    .append_operation(stable_hlo::r#return(&[value_0_op.result(0).unwrap()], location).unwrap())
                    .unwrap();
                branch_0_region.append_block(branch_0_block).unwrap();
                ...
                let value_1_op = branch_1_block
                    ...
                    .unwrap();
                branch_1_block
                    .append_operation(stable_hlo::r#return(&[value_1_op.result(0).unwrap()], location).unwrap())
                    .unwrap();
                branch_1_region.append_block(branch_1_block).unwrap();

                // Create the `case` operation.
                let index = block.argument(0).unwrap();
                let op = case(index, vec![branch_0_region.into(), branch_1_region.into()], location).unwrap();
                assert_eq!(op.branches().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
    ...
    fn test_if() {
        ...
        module
            ...
            .append_operation({
                ...
                let true_value_op = true_branch_block
                    .append_operation(
                        stable_hlo::constant(
                            context.dense_i64_elements_attribute(result_type, &[0, 0]).unwrap(),
                            location,
                    ...
                    .unwrap();
                true_branch_block
                    .append_operation(stable_hlo::r#return(&[true_value_op.result(0).unwrap()], location).unwrap())
                    .unwrap();
                true_branch_region.append_block(true_branch_block).unwrap();
                ...
                let false_value_op = false_branch_block
                    .append_operation(
                        stable_hlo::constant(
                            context.dense_i64_elements_attribute(result_type, &[1, 1]).unwrap(),
                            location,
                    ...
                    .unwrap();
                false_branch_block
                    .append_operation(stable_hlo::r#return(&[false_value_op.result(0).unwrap()], location).unwrap())
                    .unwrap();
                false_branch_region.append_block(false_branch_block).unwrap();

                // Create the `if` operation.
                let predicate = block.argument(0).unwrap();
                let if_op = r#if(predicate, true_branch_region.into(), false_branch_region.into(), location).unwrap();
                assert_eq!(if_op.predicate().unwrap(), predicate);
                assert_eq!(if_op.true_branch().unwrap().blocks().unwrap().count(), 1);
                assert_eq!(if_op.false_branch().unwrap().blocks().unwrap().count(), 1);
                assert_eq!(if_op.operands().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                assert_eq!(if_op.results().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                assert_eq!(if_op.regions().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
                let if_op = block.append_operation(if_op).unwrap();

                // Create a function that contains that `if` operation and returns its output.
                block.append_operation(func::r#return(&[if_op.result(0).unwrap()], location).unwrap()).unwrap();
                func::func(
                    "if_test",
                    func::FuncAttributes {
    ...
    fn test_while() {
        ...
        module
            ...
            .append_operation({
                ...
                let limit_value_op = condition_block
                    .append_operation(
                        stable_hlo::constant(
                    ...
                    .unwrap();
                let compare_op = condition_block
                    .append_operation(
                        stable_hlo::compare(
                ...
                condition_block.append_operation(stable_hlo::r#return(&[cond_result], location).unwrap()).unwrap();
                condition_region.append_block(condition_block).unwrap();
                ...
                let new_i_op = body_block
                    .append_operation(stable_hlo::add(body_block.argument(0).unwrap(), one_value, location).unwrap())
                    .unwrap();
                let new_i = new_i_op.result(0).unwrap();
                let new_sum_op = stable_hlo::add(body_block.argument(1).unwrap(), one_value, location).unwrap();
                let new_sum_block = body_block.append_operation(new_sum_op).unwrap();
                let new_sum = new_sum_block.result(0).unwrap();
                body_block.append_operation(stable_hlo::r#return(&[new_i, new_sum], location).unwrap()).unwrap();
                body_region.append_block(body_block).unwrap();

                // Create the `while` operation.
                let while_op = r#while(
                    block.arguments().collect::<Result<Vec<_>, _>>().unwrap().as_slice(),
                    condition_region.into(),
                    body_region.into(),
                    location,
                )
                .unwrap();
                assert_eq!(while_op.condition().unwrap().blocks().unwrap().count(), 1);
                assert_eq!(while_op.body().unwrap().blocks().unwrap().count(), 1);
                assert_eq!(while_op.operands().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
                assert_eq!(while_op.results().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
                assert_eq!(while_op.regions().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
                let while_block = block.append_operation(while_op).unwrap();

                // Create a function that contains that `while` operation and returns its outputs.
                block
                    .append_operation(
                        func::r#return(&[while_block.result(0).unwrap(), while_block.result(1).unwrap()], location)
                            .unwrap(),
                    )
                    .unwrap();
                func::func(
    ...
    fn test_select() {
        ...
        module
            ...
            .append_operation({
                ...
                assert_eq!(op.results().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                assert_eq!(op.result(0).unwrap().r#type().unwrap(), value_type);
                let op = block.append_operation(op).unwrap();
                block.append_operation(func::r#return(&[op.result(0).unwrap()], location).unwrap()).unwrap();
                func::func(
                    "select_test",
