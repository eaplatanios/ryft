...
pub fn tuple<'v, 'c: 'v, 't: 'c, V: Value<'v, 'c, 't>, L: Location<'c, 't>>(
    ...
    OperationBuilder::new("stablehlo.tuple", location)
        ...
        .and_then(|operation| unsafe {
            operation.cast().ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::tuple`"))
        })
}

/// Name of the [`Attribute`](crate::Attribute) that is used to store [`GetTupleElementOperation::index`].
...
pub trait GetTupleElementOperation<'o, 'c: 'o, 't: 'c>: Operation<'o, 'c, 't> {
    ...
    fn index(&self) -> Result<usize, Error> {
        usize::try_from(self.integer_attribute(GET_TUPLE_ELEMENT_INDEX_ATTRIBUTE)?.signless_value()).map_err(|_| {
...
pub fn get_tuple_element<'v, 'c: 'v, 't: 'c, V: Value<'v, 'c, 't>, L: Location<'c, 't>>(
    ...
    OperationBuilder::new("stablehlo.get_tuple_element", location)
        ...
        .and_then(|operation| unsafe {
            operation
                .cast()
                .ok_or_else(|| Error::invalid_argument("invalid arguments to `stable_hlo::get_tuple_element`"))
        })
}
    ...
    fn test_tuple() {
        ...
        module
            ...
            .append_operation({
                ...
                let tuple_op = tuple(&[arg_0, arg_1], location).unwrap();
                assert_eq!(tuple_op.operands().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 2);
                assert_eq!(tuple_op.results().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                assert_eq!(tuple_op.result(0).unwrap().r#type().unwrap(), tuple_type);
                let tuple_block = block.append_operation(tuple_op).unwrap();
                block
                    .append_operation(func::r#return(&[tuple_block.result(0).unwrap()], location).unwrap())
                    .unwrap();
                func::func(
                    "tuple_test",
                    func::FuncAttributes {
    ...
    fn test_get_tuple_element() {
        ...
        module
            ...
            .append_operation({
                ...
                let arg_0 = block.argument(0).unwrap();
                let arg_1 = block.argument(1).unwrap();
                let inner_tuple_op = tuple(&[arg_1], location).unwrap();
                let inner_tuple_block = block.append_operation(inner_tuple_op).unwrap();
                let inner_tuple_value = inner_tuple_block.result(0).unwrap().as_ref();
                let outer_tuple_op = tuple(&[arg_0.as_ref(), inner_tuple_value], location).unwrap();
                let outer_tuple_block = block.append_operation(outer_tuple_op).unwrap();
                let outer_tuple_result = outer_tuple_block.result(0).unwrap();
                let get_element_op = get_tuple_element(outer_tuple_result, 0, location).unwrap();
                assert_eq!(get_element_op.index().unwrap(), 0);
                assert_eq!(get_element_op.operands().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                assert_eq!(get_element_op.results().collect::<Result<Vec<_>, _>>().unwrap().into_iter().count(), 1);
                let get_element_block = block.append_operation(get_element_op).unwrap();
                block
                    .append_operation(func::r#return(&[get_element_block.result(0).unwrap()], location).unwrap())
                    .unwrap();
                func::func(
                    "nested_tuple_test",
                    func::FuncAttributes {
