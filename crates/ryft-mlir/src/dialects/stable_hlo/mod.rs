...
impl DialectHandle<'_, '_> {
    ...
    pub fn stable_hlo() -> Result<Self, Error> {
        unsafe { Self::from_c_api(mlirGetDialectHandle__stablehlo__()) }
    }
}
...
mod tests {
    ...
    fn test_stable_hlo_dialect() {
        let handle = DialectHandle::stable_hlo().unwrap();
        ...
        assert_eq!(dialect_1.namespace().unwrap(), "stablehlo");

        // Check that comparison works.
        let dialect_2 = context.load_dialect(DialectHandle::stable_hlo().unwrap()).unwrap();
