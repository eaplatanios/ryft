...
impl DialectHandle<'_, '_> {
    ...
    pub fn nvgpu() -> Self {
        unsafe { Self::from_c_api(mlirGetDialectHandle__nvgpu__()).unwrap() }
    }
}
...
mod tests {
    ...
    fn test_nvgpu_dialect() {
        let handle = DialectHandle::nvgpu();
        ...
        assert_eq!(dialect_1.unwrap().namespace().unwrap(), "nvgpu");

        let dialect_2 = context.load_dialect(DialectHandle::nvgpu());
