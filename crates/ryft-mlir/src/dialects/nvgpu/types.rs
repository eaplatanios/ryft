...
use crate::{
    Attribute, AttributeRef, Context, DialectHandle, Error, MemRefTypeRef, Type, VectorTypeRef,
...
macro_rules! nvgpu_unit_type {
    ($name:ident, $constructor:ident, $is_a:path, $get:path, $summary:literal, $mnemonic:literal) => {
        ...
        impl<'t> Context<'t> {
            ...
            pub fn $constructor<'c>(&'c self) -> Result<$name<'c, 't>, Error> {
                self.load_dialect(DialectHandle::nvgpu()?)?;
                unsafe {
                    $name::from_c_api($get(*self.handle.borrow_mut()), self)
                        .map_err(|_| Error::invalid_argument(concat!("invalid ", $mnemonic, " type")))
                }
            }
        }
    };
...
impl<'c, 't> MBarrierGroupTypeRef<'c, 't> {
    ...
    pub fn memory_space(&self) -> Result<AttributeRef<'c, 't>, Error> {
        unsafe { AttributeRef::from_c_api(mlirNvgpuMBarrierGroupTypeGetMemorySpace(self.handle), self.context) }
            .map_err(|_| Error::invalid_argument("invalid `!nvgpu.mbarrier.group` memory space"))
    }
...
impl<'c, 't> TensorMapDescriptorTypeRef<'c, 't> {
    ...
    pub fn tensor(&self) -> Result<MemRefTypeRef<'c, 't>, Error> {
        unsafe { MemRefTypeRef::from_c_api(mlirNvgpuTensorMapDescriptorTypeGetTensor(self.handle), self.context) }
            .map_err(|_| Error::invalid_argument("invalid `!nvgpu.tensormap.descriptor` tensor type"))
    }

    /// Returns the tensor map swizzle kind.
    pub fn swizzle(&self) -> Result<TensorMapSwizzleKind, Error> {
        TensorMapSwizzleKind::from_value(unsafe { mlirNvgpuTensorMapDescriptorTypeGetSwizzle(self.handle) })
            .ok_or_else(|| Error::invalid_argument("invalid `!nvgpu.tensormap.descriptor` swizzle kind"))
    }

    /// Returns the tensor map L2 promotion kind.
    pub fn l2_promo(&self) -> Result<TensorMapL2PromoKind, Error> {
        TensorMapL2PromoKind::from_value(unsafe { mlirNvgpuTensorMapDescriptorTypeGetL2Promo(self.handle) })
            .ok_or_else(|| Error::invalid_argument("invalid `!nvgpu.tensormap.descriptor` l2 promotion kind"))
    }

    /// Returns the tensor map out-of-bounds fill kind.
    pub fn oob(&self) -> Result<TensorMapOobKind, Error> {
        TensorMapOobKind::from_value(unsafe { mlirNvgpuTensorMapDescriptorTypeGetOob(self.handle) })
            .ok_or_else(|| Error::invalid_argument("invalid `!nvgpu.tensormap.descriptor` out-of-bounds fill kind"))
    }

    /// Returns the tensor map interleave kind.
    pub fn interleave(&self) -> Result<TensorMapInterleaveKind, Error> {
        TensorMapInterleaveKind::from_value(unsafe { mlirNvgpuTensorMapDescriptorTypeGetInterleave(self.handle) })
            .ok_or_else(|| Error::invalid_argument("invalid `!nvgpu.tensormap.descriptor` interleave kind"))
    }
}
...
impl<'c, 't> WarpgroupMatrixDescriptorTypeRef<'c, 't> {
    ...
    pub fn tensor(&self) -> Result<MemRefTypeRef<'c, 't>, Error> {
        unsafe { MemRefTypeRef::from_c_api(mlirNvgpuWarpgroupMatrixDescriptorTypeGetTensor(self.handle), self.context) }
            .map_err(|_| Error::invalid_argument("invalid `!nvgpu.warpgroup.descriptor` tensor type"))
    }
}
...
impl<'c, 't> WarpgroupAccumulatorTypeRef<'c, 't> {
    ...
    pub fn fragmented(&self) -> Result<VectorTypeRef<'c, 't>, Error> {
        unsafe { VectorTypeRef::from_c_api(mlirNvgpuWarpgroupAccumulatorTypeGetFragmented(self.handle), self.context) }
            .map_err(|_| Error::invalid_argument("invalid `!nvgpu.warpgroup.accumulator` fragmented vector type"))
    }
}
...
impl<'t> Context<'t> {
    ...
    pub fn nvgpu_mbarrier_group_type<'c, A: Attribute<'c, 't>>(
        ...
        memory_space: A,
        num_barriers: u32,
    ) -> Result<MBarrierGroupTypeRef<'c, 't>, Error> {
        self.load_dialect(DialectHandle::nvgpu()?)?;
        unsafe {
            ...
            .map_err(|_| Error::invalid_argument("invalid arguments to `Context::nvgpu_mbarrier_group_type`"))
        }
    }
    ...
    ) -> Result<TensorMapDescriptorTypeRef<'c, 't>, Error> {
        self.load_dialect(DialectHandle::nvgpu()?)?;
        unsafe {
            ...
            .map_err(|_| Error::invalid_argument("invalid arguments to `Context::nvgpu_tensor_map_descriptor_type`"))
        }
    }
    ...
    pub fn nvgpu_warpgroup_matrix_descriptor_type<'c>(
        &'c self,
        tensor: MemRefTypeRef<'c, 't>,
    ) -> Result<WarpgroupMatrixDescriptorTypeRef<'c, 't>, Error> {
        self.load_dialect(DialectHandle::nvgpu()?)?;
        unsafe {
            ...
            .map_err(|_| {
                Error::invalid_argument("invalid arguments to `Context::nvgpu_warpgroup_matrix_descriptor_type`")
            })
        }
    }
    ...
    ) -> Result<WarpgroupAccumulatorTypeRef<'c, 't>, Error> {
        self.load_dialect(DialectHandle::nvgpu()?)?;
        unsafe {
            ...
            .map_err(|_| Error::invalid_argument("invalid arguments to `Context::nvgpu_warpgroup_accumulator_type`"))
        }
    }
}
...
mod tests {
    ...
    fn test_device_async_token_type() {
        ...
        let token_type = context.nvgpu_device_async_token_type().unwrap();
        assert_eq!(&context, token_type.context());
        assert_eq!(token_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
    }

    #[test]
    fn test_device_async_token_type_equality() {
        let context = Context::new();
        let type_1 = context.nvgpu_device_async_token_type().unwrap();
        let type_2 = context.nvgpu_device_async_token_type().unwrap();
    ...
    fn test_device_async_token_type_display_and_debug() {
        let context = Context::new();
        test_type_display_and_debug(context.nvgpu_device_async_token_type().unwrap(), "!nvgpu.device.async.token");
    }
    ...
    fn test_device_async_token_type_casting() {
        let context = Context::new();
        test_type_casting(context.nvgpu_device_async_token_type().unwrap());
    }

    #[test]
    fn test_mbarrier_group_type() {
        let context = Context::new();
        let memory_space = context.gpu_address_space_attribute(AddressSpace::Workgroup).unwrap();
        let group_type = context.nvgpu_mbarrier_group_type(memory_space, 4).unwrap();
        assert_eq!(&context, group_type.context());
        assert_eq!(group_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
        assert_eq!(group_type.memory_space().unwrap(), memory_space);
    ...
    fn test_mbarrier_group_type_equality() {
        let context = Context::new();
        let memory_space = context.gpu_address_space_attribute(AddressSpace::Workgroup).unwrap();
        let type_1 = context.nvgpu_mbarrier_group_type(memory_space, 4).unwrap();
        let type_2 = context.nvgpu_mbarrier_group_type(memory_space, 4).unwrap();
        assert_eq!(type_1, type_2);

        let type_2 = context.nvgpu_mbarrier_group_type(memory_space, 2).unwrap();
        assert_ne!(type_1, type_2);
    ...
    fn test_mbarrier_group_type_casting() {
        let context = Context::new();
        let memory_space = context.gpu_address_space_attribute(AddressSpace::Workgroup).unwrap();
        test_type_casting(context.nvgpu_mbarrier_group_type(memory_space, 4).unwrap());
    }

    #[test]
    fn test_mbarrier_token_type() {
        let context = Context::new();
        let token_type = context.nvgpu_mbarrier_token_type().unwrap();
        assert_eq!(&context, token_type.context());
        assert_eq!(token_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
    }
    ...
    fn test_mbarrier_token_type_equality() {
        let context = Context::new();
        let type_1 = context.nvgpu_mbarrier_token_type().unwrap();
        let type_2 = context.nvgpu_mbarrier_token_type().unwrap();
        assert_eq!(type_1, type_2);
    ...
    fn test_mbarrier_token_type_display_and_debug() {
        let context = Context::new();
        test_type_display_and_debug(context.nvgpu_mbarrier_token_type().unwrap(), "!nvgpu.mbarrier.token");
    }
    ...
    fn test_mbarrier_token_type_casting() {
        let context = Context::new();
        test_type_casting(context.nvgpu_mbarrier_token_type().unwrap());
    }
    ...
    fn test_tensor_map_descriptor_type() {
        ...
        assert_eq!(descriptor_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
        assert_eq!(descriptor_type.tensor().unwrap(), tensor);
        assert_eq!(descriptor_type.swizzle().unwrap(), TensorMapSwizzleKind::Swizzle128B);
        assert_eq!(descriptor_type.l2_promo().unwrap(), TensorMapL2PromoKind::L2Promo64B);
        assert_eq!(descriptor_type.oob().unwrap(), TensorMapOobKind::Zero);
        assert_eq!(descriptor_type.interleave().unwrap(), TensorMapInterleaveKind::None);
    }
    ...
    fn test_warpgroup_matrix_descriptor_type() {
        ...
        let descriptor_type = context.nvgpu_warpgroup_matrix_descriptor_type(tensor).unwrap();
        assert_eq!(&context, descriptor_type.context());
        assert_eq!(descriptor_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
        assert_eq!(descriptor_type.tensor().unwrap(), tensor);
    }
    ...
    fn test_warpgroup_matrix_descriptor_type_equality() {
        ...
        let type_1 = context.nvgpu_warpgroup_matrix_descriptor_type(memref_1).unwrap();
        let type_2 = context.nvgpu_warpgroup_matrix_descriptor_type(memref_1).unwrap();
        assert_eq!(type_1, type_2);

        let type_2 = context.nvgpu_warpgroup_matrix_descriptor_type(memref_2).unwrap();
        assert_ne!(type_1, type_2);

        let context = Context::new();
        let location = context.unknown_location();
        let memref = context.mem_ref_type(context.float16_type(), &[Size::Static(64)], None, None, location).unwrap();
        let type_2 = context.nvgpu_warpgroup_matrix_descriptor_type(memref).unwrap();
        assert_ne!(type_1, type_2);
    }
    ...
    fn test_warpgroup_matrix_descriptor_type_casting() {
        ...
        let location = context.unknown_location();
        let tensor = context.mem_ref_type(context.float16_type(), &[Size::Static(64)], None, None, location).unwrap();
        test_type_casting(context.nvgpu_warpgroup_matrix_descriptor_type(tensor).unwrap());
    }
    ...
    fn test_warpgroup_accumulator_type() {
        ...
        let accumulator_type = context.nvgpu_warpgroup_accumulator_type(fragmented).unwrap();
        assert_eq!(&context, accumulator_type.context());
        assert_eq!(accumulator_type.dialect().unwrap().namespace().unwrap(), "nvgpu");
        assert_eq!(accumulator_type.fragmented().unwrap(), fragmented);
    }
    ...
    fn test_warpgroup_accumulator_type_equality() {
        ...
        let type_1 = context.nvgpu_warpgroup_accumulator_type(vector_1).unwrap();
        let type_2 = context.nvgpu_warpgroup_accumulator_type(vector_1).unwrap();
        assert_eq!(type_1, type_2);

        let type_2 = context.nvgpu_warpgroup_accumulator_type(vector_2).unwrap();
        assert_ne!(type_1, type_2);

        let context = Context::new();
        let location = context.unknown_location();
        let vector = context.vector_type(context.float32_type(), &[VectorTypeDimension::Fixed(64)], location).unwrap();
        let type_2 = context.nvgpu_warpgroup_accumulator_type(vector).unwrap();
        assert_ne!(type_1, type_2);
    }
    ...
    fn test_warpgroup_accumulator_type_casting() {
        ...
        let fragmented =
            context.vector_type(context.float32_type(), &[VectorTypeDimension::Fixed(64)], location).unwrap();
        test_type_casting(context.nvgpu_warpgroup_accumulator_type(fragmented).unwrap());
    }
}
