...
use ryft_core::sharding::{DeviceMesh, LogicalMesh, MeshAxis, MeshAxisType, MeshDevice, Sharding, ShardingDimension};
...
fn test_device_put_visualizes_uneven_1d_partitioning() {
    ...
    let array = Array::from_host_buffer(
        ...
        [5usize],
        DataType::F32,
        ArrayPlacement::new(mesh.clone(), sharding),
    )
    .unwrap();
...
fn test_array_put_reshards_fully_addressable_array() {
    ...
    let source_array = Array::from_host_buffer(
        ...
        [5usize],
        DataType::F32,
        ArrayPlacement::new(source_mesh, source_sharding),
    )
    .unwrap();
    ...
    let moved_array = source_array
        .to_placement(&client, ArrayPlacement::new(target_mesh.clone(), target_sharding))
        .unwrap();
...
fn test_array_put_copies_matching_local_shards_without_full_source_addressability() {
    ...
    let source_array = Array::from_shape_and_placement(
        vec![4usize],
        DataType::F32,
        ArrayPlacement::new(mesh.clone(), sharding.clone()),
        vec![local_source_buffer],
    )
    .unwrap();

    let copied_array = source_array.to_placement(&client, ArrayPlacement::new(mesh.clone(), sharding)).unwrap();
...
fn test_plan_exact_shard_put_uses_cross_host_send_and_receive_for_remote_exact_moves() {
    ...
    let target_mesh = DeviceMesh::new(
        LogicalMesh::new(vec![MeshAxis::new("x", 2, MeshAxisType::Auto).unwrap()]).unwrap(),
        vec![MeshDevice::new(remote_device_id, 1), MeshDevice::new(local_device_id, client.process_index().unwrap())],
    )
    .unwrap();
    let target_sharding =
        Sharding::new(target_mesh.logical_mesh.clone(), vec![ShardingDimension::sharded(["x"])]).unwrap();
...
fn test_array_put_rejects_non_addressable_source_shards() {
    ...
    assert!(matches!(
        source_array.to_placement(&client, ArrayPlacement::new(target_mesh, target_sharding)),
...
fn test_device_put_broadcasts_root_placement_over_array_tuple() {
    ...
    let first_source_array = Array::from_host_buffer(
        ...
        [5usize],
        DataType::F32,
        ArrayPlacement::new(source_mesh.clone(), source_sharding.clone()),
    )
    .unwrap();
    ...
    let moved_arrays = device_put(
        ...
        DevicePutOptions {
            device: Some(DevicePutTarget::placement(target_mesh.clone(), target_sharding.clone())),
...
fn test_array_to_device_preserves_same_partially_addressable_placement() {
    ...
    let copied_array =
        source_array.to_device(&client, DevicePutTarget::placement(mesh.clone(), sharding.clone())).unwrap();
    let expected_visualization =
        format!("┌─────┬─────┐\n│{:^5}│{:^5}│\n└─────┴─────┘", local_device_id, remote_device_id);
...
fn test_device_put_rejects_mismatched_src_for_array_leaf() {
    ...
    .unwrap();
    let expected_src = DevicePutTarget::device(MeshDevice::new(source_device.id + 1, 0)).resolve(1).unwrap();
    ...
    assert!(matches!(
        device_put(
            ...
            source_array,
            DevicePutOptions {
                src: Some(DevicePutTarget::device(MeshDevice::new(source_device.id + 1, 0))),
                ..DevicePutOptions::defaults()
            },
        ),
        Err(ArrayError::SourcePlacementMismatch { expected, actual })
            if expected == expected_src
                && actual == ArrayPlacement::new(source_mesh, source_sharding),
    ));
}
...
fn test_shard_metadata_rank_mismatch() {
    ...
    let sharding =
        Sharding::new(logical_mesh, vec![ShardingDimension::sharded(["x"]), ShardingDimension::sharded(["y"])])
            .unwrap();
    assert!(matches!(
        ShardLayout::new(&[8usize], &mesh, &sharding),
...
fn test_shard_metadata_unconstrained_is_ignored() {
    ...
    let sharding =
        Sharding::new(logical_mesh, vec![ShardingDimension::sharded(["x"]), ShardingDimension::unconstrained()])
            .unwrap();
    let layout = ShardLayout::new(&[8, 6], &mesh, &sharding).unwrap();
...
fn test_shard_metadata_uneven_partitioning() {
    ...
    let sharding = Sharding::new(mesh.logical_mesh.clone(), vec![ShardingDimension::sharded(["x"])]).unwrap();
    let layout = ShardLayout::new(&[5], &mesh, &sharding).unwrap();
...
fn test_shard_metadata_multi_axis_single_dimension_partitioning() {
    ...
    let layout = ShardLayout::new(&[10], &mesh, &sharding).unwrap();
...
fn test_shard_metadata_mesh_mismatch_reports_expected_and_actual_meshes() {
    ...
    assert!(matches!(
        ShardLayout::new(&[8], &mesh, &sharding),
