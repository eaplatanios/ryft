...
mod tests {
    ...
    fn test_to_mlir_module_for_plain_program_lowers_fused_linear_while_with_residual_injections() {
        ...
        let primal_entry = builder
            ...
            .unwrap()[0];
        // The transposition ingredients are inert for lowering (only the fused `operation` is consumed), so they are
        // built as trivial identity programs over empty residual sets.
        let identity_primal_body = {
            let mut builder =
                ryft_core::programs::ProgramBuilder::<ArrayType, TestArray, ArrayOperation<TestArray, ArrayType>>::new(
                );
            let input = builder.add_input(scalar_f64.clone());
            builder
                .build::<Vec<TestArray>, Vec<TestArray>>(vec![input], vec![Placeholder], vec![Placeholder])
                .unwrap()
        };
        let identity_body_pushforward = {
            let mut builder = ryft_core::programs::ProgramBuilder::<
                ArrayType,
                TestArray,
        ...
        let while_outputs = builder
            .add_instruction(
                DirectLinearOperation::While {
                    operation: Box::new(fused_while),
                    primal_body: Box::new(identity_primal_body),
                    body_pushforward: Box::new(identity_body_pushforward),
                    residual_types: vec![],
                    loop_invariant_residuals: vec![],
                    initial_state: vec![],
                },
                vec![primal_entry, tangent_input],
            )
            .unwrap()
            .to_vec();
